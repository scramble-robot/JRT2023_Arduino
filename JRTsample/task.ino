/*
【定義】
　SW_ROLLER：ローラをON/OFFするボタン．押すと1，離すと0になる．
　SW_SHOT：射出するボタン．押すと1，離すと0になる．
　SW_OTHER：未使用ボタン．押すと1，離すと0になる．
　SHOT_FRONT_SW：装填機構の前進端側リミットスイッチ．押すと0，離すと1になる．
　SHOT_BACK_SW：装填機構の後退端側リミットスイッチ．押すと0，離すと1になる．

【変数】
RxData[0]~RxData[3]:コントローラから受信したデータが入った配列
  RxData[0]：左タイヤのアナログ量(-100～100)
  RxData[1]：右タイヤのアナログ量(-100～100)
  RxData[2]：ピッチのアナログ量(-100～100)
  RxData[3]：スイッチ
ShotSeq：射出のシーケンス管理用変数

【関数】
MotorON(motor, pwm)：モータを回すための関数　motor：回すモータ　pwm：-100~100
  WHEEL_L：左タイヤ
  WHEEL_R：右タイヤ
  SHOT：装填用モータ
  例)　MotorON(WHEEL_L, -50);
MotorOFF(motor)：モータを停止するための関数　motor：停止するモータ

ServoON(servo, angle)：サーボを動かすための関数　servo：動かすサーボ　angle：90~180
  PITCH：ピッチ用のサーボ
  例)　ServoON(PITCH, 100);
  
*/
// 足回りの制御
void Wheel(void){

}

// 射出のシーケンス
void Shot(void){

}

// 射出の角度調整
void Pitch(void){

}